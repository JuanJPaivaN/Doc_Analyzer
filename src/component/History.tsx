...
const History = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Historial de Análisis</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          {/* Lista de análisis (ocupando 2/5 del espacio) */}
            ...
            {analyses.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                {analyses.map((analysis) => (
                  <li 
                    key={analysis.id} 
                    className={`py-4 cursor-pointer hover:bg-gray-50 ${selectedAnalysis?.id === analysis.id ? 'bg-blue-50' : ''}`}
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {truncateText(analysis.documentContent)}
                    </p>
                  </li>
                ))}
              </ul>
          ...
          {/* Detalle del análisis seleccionado (ocupando 3/5 del espacio) */}
